...
var resizePizzas = function (size) {
  ...
  //##STUDENT COMMENT instead of creating an array inside the loop, i create it once, i get the needed information
  //from the first element, as there's no need to loop through all of them, and i left only the code that updates the elements
  //with the changes values in the loop
  function changePizzaSizes(size) {
    var pizzaContainersArray = document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(pizzaContainersArray[0], size);
    var newwidth = (pizzaContainersArray[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < pizzaContainersArray.length; i++) {
      pizzaContainersArray[i].style.width = newwidth;
...
// ## STUDENT COMMENT I moved all the code that needs to be run once outside of the loop; I'm guessing more can be done Here
//but after trying few things i didn't notice any real improvement'
...
document.addEventListener('DOMContentLoaded', function () {
  var cols = 8;
  var s = 256;
  //## STUDENT COMMENT i reduced the pizza number to 22 instead of 200, no need for that many pizzas
